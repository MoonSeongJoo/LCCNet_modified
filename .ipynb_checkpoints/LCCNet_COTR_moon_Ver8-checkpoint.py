...
#         plt.subplot(211)
#         plt.imshow(torchvision.utils.make_grid(rgb_input ,normalize=True).permute(1,2,0).cpu().numpy())
#         plt.title("RGB_input", fontsize=22)
#         plt.axis('off')

#         plt.subplot(212)
#         plt.imshow(torchvision.utils.make_grid(depth_input ,normalize=True).permute(1,2,0).cpu().numpy() , cmap='magma')
...
#         plt.figure(figsize=(10, 10))
#         plt.imshow(torchvision.utils.make_grid(rgb_cotr_input ,normalize=True).permute(1,2,0).cpu().numpy() , cmap="magma")
#         plt.title("RGB_depth_pred", fontsize=22)
#         plt.axis('off')
        
        rgb_cotr_input = rgb_cotr_input.permute(0,2,3,1)
#         print ('merged_depth_img_shape=' , merged_depth_img.shape)
        sbs_img = self.two_images_side_by_side(rgb_cotr_input, depth_input)
        sbs_img = torch.from_numpy(sbs_img).permute(0,3,1,2)
        sbs_img = tvtf.normalize(sbs_img, (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
#         print ('sbs_img_shape=' , sbs_img.shape)
        
#         ######### display depth image each sensor#############

#         plt.figure(figsize=(10, 10))
# #         plt.subplot(311)
#         plt.imshow(torchvision.utils.make_grid(sbs_img ,normalize=True).permute(1,2,0).cpu().numpy() , cmap='magma')
#         plt.title("sbs_img_input", fontsize=22)
#         plt.axis('off')
...
#         rgb_cotr_input = rgb_cotr_input.squeeze(0).permute(1,2,0)
...
#         merged_depth_img = transforms.ToTensor()(rgb_cotr_input_np_color).unsqueeze(0).permute(0,1,2,3)
#         print ("merged_depth_img_shape =" , merged_depth_img.shape)
#         lidar_cotr_input_np_gray = lidar_cotr_input.cpu().numpy()

        ########## display merged depth image #############

#         plt.figure(figsize=(10, 10))
#         plt.subplot(311)
#         plt.imshow(torchvision.utils.make_grid(merged_depth_img ,normalize=True).permute(1,2,0).cpu().numpy() , cmap='magma')
#         plt.imshow(rgb_cotr_input_np_color , cmap='magma')
#         plt.title("depth_cotr_input", fontsize=22)
#         plt.axis('off')
