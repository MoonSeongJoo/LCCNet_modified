...
#         plt.savefig('dense_depth_map_exam.png', bbox_inches='tight', pad_inches = 0 , edgecolor = 'white' 
...
#         plt.show()
        ...
        rgb_np_resized = cv2.resize(rgb_np, (512,256), interpolation=cv2.INTER_LINEAR)
        depth_img_np_resized = cv2.resize(depth_img_np, (512,256), interpolation=cv2.INTER_LINEAR)
        depth_gt_np_resized = cv2.resize(depth_gt_np, (512,256), interpolation=cv2.INTER_LINEAR)        
...
#         print ("corrs max value=" , np.max(corrs))
#         print ("corrs min value=" , np.min(corrs))
#         print ("-------gt_points_index value---------" ,  gt_points_index)
#         print ("-------points_index value------------" ,  points_index)
#         valid_mask = gt_points_index == points_index
#         print ("-------valid_mask_value---------" ,  valid_mask)
#         corrs = corrs[valid_mask]
#         print ("-------corrs shape--------------" ,  corrs.shape)
        corrs = self.trim_corrs(corrs)
#         corrs_MinMax_scaled = minmax_scale(corrs, axis=0, copy=True)
#         corrs_MinMax_scaled = torch.from_numpy(corrs)
        corrs = torch.tensor(corrs)
        corrs[:, 0] = (0.5*corrs[:, 0])/1280
        corrs[:, 1] = (1*corrs[:, 1])/384
        corrs[:, 2] = (0.5*corrs[:, 2])/1280 + 0.5        
        corrs[:, 3] = (1*corrs[:, 3])/384        
#         print ("corrs max value=" , np.max(corrs[:,1]))
#         print ("corrs min value=" , np.min(corrs[:,1]))        
