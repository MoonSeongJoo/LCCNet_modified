...
from COTR.COTR_models.cotr_model_moon_Ver4 import build
...
#                 print("-c-" , c )
    ...
    def forward(self, rgb_input, lidar_input ,query_input , corr_target):
...
#         rgb_input_np = rgb_input.cpu().numpy()
        ...
        img_reverse = torch.cat([img[..., 512:], img[..., :512]], axis=-1)
...
#         corrs = self.corr(img, query)['pred_corrs']
...
#         pred_corrs = self.draw_corrs(img_cpu, pred_corrs)
        target_corrs = self.draw_corrs(img_cpu, target_corrs)
        ...
        cycle = self.corr(img_reverse, query_reverse)['pred_corrs']
        cycle[..., 0] = cycle[..., 0] - 0.5
