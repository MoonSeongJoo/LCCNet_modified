...
class DatasetLidarCameraKittiOdometry(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_gt, uv = self.lidar_project_depth(pc_in, calib , real_shape) # image_shape
        depth_gt /= 80.
        ...
        depth_img, uv = self.lidar_project_depth(pc_rotated, calib , real_shape) # image_shape
        depth_img /= 80.
        ...
        if self.split == 'test':
            sample = {'rgb': rgb_torch, 'point_cloud': pc_in, 'calib': calib,
                      ...
                      'initial_RT': initial_RT , 'depth_img' : lidar_torch , 'depth_gt' : lidar_gt_torch  , 
                      'depth_img_raw' : lidar_raw_torch , 'depth_gt_raw' : lidar_raw_gt_torch }
        else:
            sample = {'rgb': rgb_torch, 'point_cloud': pc_in, 'calib': calib,
                      'tr_error': T_torch, 'rot_error': R_torch, 'seq': int(seq),
                      'rgb_name': rgb_name, 'item': item, 'extrin': RT_torch , 
                      'depth_img' : lidar_torch , 'depth_gt' : lidar_gt_torch,
                     'depth_img_raw' : lidar_raw_torch , 'depth_gt_raw' : lidar_raw_gt_torch }
