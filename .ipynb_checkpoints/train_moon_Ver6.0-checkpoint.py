...
def config():
    ...
    max_r = 10 # 20.0, 10.0, 5.0,  2.0,  1.0
    batch_size = 2  # 120
    num_worker = 10
    network = 'Res_f1'
    optimizer = 'adam'
    resume = True
    weights = '/root/work/LCCNet_Moon/checkpoints/kitti/odom/val_seq_03/models/checkpoint_r10.00_t0.20_e2_0.279.tar'
#     weights = None
    rescale_rot = 0.1
    rescale_transl = 100
    precision = "O0"
...
#             dense_depth_img_input = dense_depth_img_input.permute(0,3,1,2)
            ...
            loss, trasl_e, rot_e, R_predicted,  T_predicted = val(model, rgb_input, corrs_input ,
                                                                  sample['tr_error'], sample['rot_error'],
                                                                  loss_fn, sample['point_cloud'], _config['loss'])
