...
class LCCNet(nn.Module):
    ...
    def __init__(self, corr_net, image_size =64, use_feat_from=1, md=4, use_reflectance=False, dropout=0.0,
        ...
        #self.fc1 = nn.Linear(fc_size * 4, 512)
        self.fc0_rot_aggr = nn.Linear(self.num_kp * 6 , 1024) # select numer of corresepondence matching point * 2 shape[0] # ========= number of kp (self.num_kp) * 4 ===========
        self.fc0_tarsl_aggr = nn.Linear(self.num_kp * 6 , 1024) # select numer of corresepondence matching point * 2 shape[0] # ========= number of kp (self.num_kp) * 4 ===========
        #self.fc1_trasl = nn.Linear(512, 256)
...
#             cycle_loss = torch.nn.functional.mse_loss(cycle[mask], query[mask])        
        ...
        x = self.mish(concatenated_tensors)
        x = x.view(x.shape[0], -1)
        x = self.dropout(x)
        # x = x.to('cuda')
        # x = x.float()
        aggr_rot_x = self.mish(self.fc0_rot_aggr(x))
        aggr_transl_x = self.mish(self.fc0_tarsl_aggr(x))

        transl = self.mish(self.fc0_trasl(aggr_transl_x))
        rot = self.mish(self.fc0_rot(aggr_rot_x))
        transl = self.mish(self.fc1_trasl(transl))
        rot = self.mish(self.fc1_rot(rot))
