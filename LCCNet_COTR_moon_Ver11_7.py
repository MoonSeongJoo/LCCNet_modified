...
class CalibActionHead(nn.Module):
    def __init__(self):
        ...
        self.input_dim = 1*900 # batch * (number of keypoint*corr columns)
        self.head_dim = 128
...
#         plt.axis('off')
        ...
        corrs_pred ,cycle, mask = self.corr(sbs_img, query_input)
...
#             cycle_loss = torch.nn.functional.mse_loss(cycle[mask], query[mask])        
        ...
        feature_emb = torch.cat((concat_pred_corrs, concat_pred_corrs_diff), dim=-1)  # shape (batch, 100, 6) 
