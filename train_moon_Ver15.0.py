...
from DatasetLidarCamera_Ver15_0 import DatasetLidarCameraKittiOdometry ,DatasetLidarCameraKittiRaw,DatasetLidarCameraNuscenes
from losses_Ver15_0 import DistancePoints3D, GeometricLoss, L1Loss, ProposedLoss, CombinedLoss
...
from image_processing_unit_Ver15_0 import (lidar_project_depth , corr_gen , corr_gen_withZ , dense_map 
                                           , colormap,lidar_project_depth_nuscenes,transform_gt)
from LCCNet_COTR_moon_Ver15_0 import DepthCalibTranformer
...
def config():
    ...
    # weights = './checkpoints/kitti/odom/val_seq_2011_09_26_drive_0005_sync/models/checkpoint_r10.00_t0.25_e87_2.330.tar'
    weights = None
...
# CNN training
@ex.capture
def train(model, optimizer, rgb_input, dense_depth_input, corrs , target_transl, target_rot, loss_fn, point_clouds, loss, dataset_type):
    model.train()
...
#     plt.axis('off')
    ...
    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err , corr_target , corr_pred , queries ,cycle, mask, dataset_type)
    else:
...
# CNN test
@ex.capture
def val(model, rgb_input, dense_depth_input ,corrs ,target_transl, target_rot, loss_fn, point_clouds, loss, dataset_type):
    model.eval()
...
def main(_config, _run, seed):
    ...
    elif type(_config['val_sequence']) is str:
    ...
    if _config['dataset_type'] == 'kitti' :
        ...
        dataset_val = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
                                    split='val', use_reflectance=_config['use_reflectance'],
                                    val_sequence=_config['val_sequence']) #for kitti data instance
        # dataset_val = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
        ...
        nusc_train = NuScenes(version='v1.0-trainval', dataroot=_config['data_folder'], verbose=True)
        nusc_val = NuScenes(version='v1.0-mini', dataroot=_config['data_folder'], verbose=True)    
        
        dataset_class = DatasetLidarCameraNuscenes # Nuscenes dataset
        dataset_train = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
                                    split='train',val_sequence=_config['val_sequence']) # for nuscenes data instance
        dataset_val = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
                                    split='val',val_sequence=_config['val_sequence']) # for nuscenes data instance
...
#     plt.axis('off'); 
    ...
    #     ### old netwrok define ####
    #     model_old = LCCNet_old1(input_size, use_feat_from=feat, md=md,
    ...
    if _config['weights'] is not None:
        ...
        # corr_net_loaded = model_old.corr
        print("old model only corr network loaded")
        # for name, param_old in model_old.corr.named_parameters():
    ...
    if _config['regressor_init'] == 'init' :
        ...
        for m in model.fc0_tarsl_aggr.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
                if m.bias is not None:
                    m.bias.data.zero_()
        for m in model.fc0_rot_aggr.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
                if m.bias is not None:
                    m.bias.data.zero_()
        for m in model.fc1_trasl.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
                if m.bias is not None:
                    m.bias.data.zero_()
        for m in model.fc1_rot.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
                if m.bias is not None:
                    m.bias.data.zero_()
    ...
    if _config['all_net_init'] == 'init' :
        
        for m in model.corr.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
                if m.bias is not None:
                    m.bias.data.zero_()
        ...
        for m in model.fc2_rot.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                ...
                if m.bias is not None:
                    m.bias.data.zero_()
        print (f"all network init : {_config['all_net_init']}")
    elif _config['all_net_init'] == None :
        print (f"all network init : {_config['all_net_init']}")
...
#         scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20, 50, 70], gamma=0.3)
    ...
    if _config['optimizer'] == 'adamW':
        ...
        # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50,100,150,200,300,400,500], gamma=0.5)
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                if _config['dataset_type'] =='kitti':
                    ...
                    nusc_sample = nusc_train.sample[idx]
                    # Load and transform lidar data.
                    lidar_data = nusc_train.get('sample_data', nusc_sample['data']['LIDAR_TOP'])
                    lidar_path = nusc_train.get_sample_data_path(nusc_sample['data']['LIDAR_TOP'])
                    # lidar_points = LidarPointCloud.from_file(lidar_path)
                    lidar_points, _ = LidarPointCloud.from_file_multisweep(nusc_train, nusc_sample, 'LIDAR_TOP', 'LIDAR_TOP', nsweeps=10)
                    
                    # 카메라 데이터를 로드합니다.
                    cam_data = nusc_train.get('sample_data', nusc_sample['data']['CAM_FRONT'])
                    cam_path = nusc_train.get_sample_data_path(nusc_sample['data']['CAM_FRONT'])
                    im = cv2.imread(cam_path)
                    img = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                    rgb = transforms.ToTensor()(img).to(device)

                    lidar_points = transform_gt(nusc_train, lidar_data, lidar_points ,cam_data)

                    pc_transformed_gt = torch.from_numpy(lidar_points.points).cuda()
                    
                    depth_gt, gt_uv , gt_z , gt_points_index  = lidar_project_depth_nuscenes(nusc_train, pc_transformed_gt, cam_data, real_shape)
                    depth_gt /= _config['max_depth']
                ...
                # homogenous_pc= torch.cat([pc_transformed_gt, ones], dim=0) 
                    ...
                    pc_rotated = rotate_back(pc_transformed_gt, RT)  # Pc` = RT * Pc

                    depth_img, uv , z , points_index  = lidar_project_depth_nuscenes(nusc_train, pc_rotated, cam_data, real_shape)
                    depth_img /= _config['max_depth']
            ...
            if _config['dataset_type'] =='kitti': 
                loss, R_predicted,  T_predicted = train(model, optimizer, rgb_input, lidar_input , corrs_input ,
                                        ...
                                        loss_fn, sample['point_cloud'], _config['loss'],_config['dataset_type'])
            elif _config['dataset_type'] == 'nuscenes' :
                loss, R_predicted,  T_predicted = train(model, optimizer, rgb_input, lidar_input , corrs_input ,
                        sample['tr_error'], sample['rot_error'],
                        loss_fn, pc_transformed_gt, _config['loss'],_config['dataset_type'])
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                if _config['dataset_type'] == 'kitti':
                    ...
                    real_shape = [sample['rgb'][idx].shape[0], sample['rgb'][idx].shape[1], sample['rgb'][idx].shape[2]] #[height:width:channel/ 900,1600,3]
                    nusc_sample = nusc_val.sample[idx]
                    # Load and transform lidar data.
                    lidar_data = nusc_val.get('sample_data', nusc_sample['data']['LIDAR_TOP'])
                    lidar_path = nusc_val.get_sample_data_path(nusc_sample['data']['LIDAR_TOP'])
                    # lidar_points = LidarPointCloud.from_file(lidar_path)
                    lidar_points, _ = LidarPointCloud.from_file_multisweep(nusc_val, nusc_sample, 'LIDAR_TOP', 'LIDAR_TOP', nsweeps=10)
                    
                    # 카메라 데이터를 로드합니다.
                    cam_data = nusc_val.get('sample_data', nusc_sample['data']['CAM_FRONT'])
                    cam_path = nusc_val.get_sample_data_path(nusc_sample['data']['CAM_FRONT'])
                    im = cv2.imread(cam_path)
                    img = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                    rgb = transforms.ToTensor()(img).to(device)

                    lidar_points = transform_gt(nusc_val, lidar_data, lidar_points ,cam_data)

                    pc_transformed_gt = torch.from_numpy(lidar_points.points).cuda()
                    
                    depth_gt, gt_uv , gt_z , gt_points_index  = lidar_project_depth_nuscenes(nusc_val, pc_transformed_gt, cam_data, real_shape)
                    depth_gt /= _config['max_depth']
                ...
                # homogenous_pc= torch.cat([pc_transformed_gt, ones], dim=0) 
                    ...
                    depth_img, uv , z , points_index  = lidar_project_depth_nuscenes(nusc_val, pc_rotated, cam_data, real_shape)
                    depth_img /= _config['max_depth']
                ...
                dense_depth_img_color_gt = F.pad(dense_depth_img_color_gt, shape_pad).cuda()
            ...
            lidar_input_gt = F.interpolate(lidar_input_gt, size=[192, 640], mode="bilinear")
            # lidar_input = torch.cat((lidar_input,lidar_input,lidar_input), 1)
            corrs_input = torch.stack(corrs_input)
