...
from DatasetLidarCamera_Ver14_0 import DatasetLidarCameraKittiOdometry ,DatasetLidarCameraKittiRaw
from losses_Ver14_0 import DistancePoints3D, GeometricLoss, L1Loss, ProposedLoss, CombinedLoss
...
from image_processing_unit_Ver14_0 import (lidar_project_depth , corr_gen , corr_gen_withZ , dense_map , colormap ,
                                           two_images_side_by_side ,two_images_side_by_side_for_gray,
                                           two_images_side_by_side_for_gray_scale,random_mask)
from LCCNet_COTR_moon_Ver14_0 import DepthCalibTranformer , MonoDelsNet
...
def config():
    ...
    # val_sequence = '2011_09_26_drive_0005_sync' # for kitti data raw
    epochs = 1000
...
=======
    ...
    weights = None
>>>>>>> 754542f0f50265e00230b55302c9dff58e863ea0
...
#         scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20, 50, 70], gamma=0.3)
    ...
    ########### traing batch #############################
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            rgb_min_value = torch.min(rgb_input)
            rgb_max_value = torch.max(rgb_input)
            rgb_mean_value = torch.mean(rgb_input)
            rgb_variance_value = torch.var(rgb_input)
            ...
            normalized_rgb_input = (rgb_input - rgb_min_value) / (rgb_max_value - rgb_min_value)
            ...
            # rgbdepth_pred_input = rgb_depth_pred # for gray 

            rgb_pred_min_value = torch.min(rgbdepth_pred_input)
            rgb_pred_max_value = torch.max(rgbdepth_pred_input)
            rgb_pred_mean_value = torch.mean(rgbdepth_pred_input)
            rgb_pred_variance_value = torch.var(rgbdepth_pred_input)
            ...
            normalized_rgbdepth_pred_input = (rgbdepth_pred_input - rgb_pred_min_value) / (rgb_pred_max_value - rgb_pred_min_value)
            ...
            # plt.subplot(211)
            # plt.imshow(torchvision.utils.make_grid(normalized_rgbdepth_pred_input , normalize = True).permute(1,2,0).cpu())
            # plt.title("rgb pred", fontsize=22)
            # plt.axis('off') 
            # plt.show()        

            # plt.figure(figsize=(20, 40))
            # plt.subplot(212)
            # plt.imshow(torchvision.utils.make_grid(normalized_lidar_projection_input, normalize = True).permute(1,2,0).cpu())
            # plt.title("lidar projection", fontsize=22)
            # plt.axis('off')
            # plt.show()
            ...
            # ####### display input signal(for gray) #########   
            # # sbs_img_disp =torchvision.utils.make_grid(sbs_img_rgb).cpu().numpy().transpose((1,2,0))[:,:,0] 
            # sbs_img_disp =torchvision.utils.make_grid(sbs_img , normalize = True).permute(1,2,0).cpu()   
            # plt.figure(figsize=(20, 40))
            ...
            corrs_input[:,:,0] = corrs_input[:,:,0]/2    # scaling points for sbs image resizing
            corrs_input[:,:,1] = corrs_input[:,:,1]/2 
            corrs_input[:,:,3] = corrs_input[:,:,3]/2 + 0.5 # scaling points for sbs image resizing
            ...
            if _config['loss'] == 'points_distance' or _config['loss'] == 'combined':
                ...
                local_quatenion_loss += loss['loss_rot'].item()
                local_tmae_loss += loss['loss_t_mae'].item()
                local_trans_loss += loss['clone_loss_trans'].item()
                local_rot_loss += loss['clone_loss_rot'].item()
            ...
            train_quatenion_loss = local_quatenion_loss/50
...
# #                     train_writer.add_scalar("Loss_Total", loss.item(), train_iter)
            ...
            if batch_idx % 50 == 0 and batch_idx != 0:
                ...
                print(f'Iter {batch_idx}/{len(TrainImgLoader)} training loss = {train_local_loss:.4f}, '
                      ...
                      f'loss of local rot = {train_rot_loss:.6f} ,'
                      f'loss of local trans = {train_trans_loss:.6f} ,'
                      f'loss of tmae = {train_tmae_loss:.4f} ,'
                      f'loss of quatenion = {train_quatenion_loss:.4f} ,'
            ...
            if _config['loss'] == 'points_distance' or _config['loss'] == 'combined':
                ...
                sum_quatenion_loss += loss['loss_rot'] * len(sample['rgb'])
                sum_tmae_loss += loss['loss_t_mae'].item() * len(sample['rgb'])
                sum_trans_loss += loss['clone_loss_trans'] * len(sample['rgb'])
                sum_rot_loss += loss['clone_loss_rot'].item() * len(sample['rgb'])
        ...
        print('epoch %d total local trans loss = %.4f' % (epoch, sum_trans_loss / len(dataset_train)))
        print('epoch %d total local rot loss = %.4f' % (epoch, sum_rot_loss / len(dataset_train)))
        print('epoch %d total tmae loss = %.4f' % (epoch, sum_tmae_loss / len(dataset_train)))
        print('epoch %d total quaternion loss = %.4f' % (epoch, sum_quatenion_loss / len(dataset_train)))
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            # plt.subplot(212)
            # plt.imshow(torchvision.utils.make_grid(sbs_img_masked).permute(1,2,0).cpu().numpy())
