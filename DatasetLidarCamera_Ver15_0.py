...
class DatasetLidarCameraKittiRaw(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, use_reflectance=False,
        ...
        date_list = ['2011_09_26'] # 09_26 only
        data_drive_list = ['0001', '0002', '0004', '0016', '0009','0011','0013','0014','0015','0017','0018','0019','0020','0022','0023','0027','0028','0029','0032','0035','0036','0039','0046','0048','0051','0052','0056','0057','0059']
        self.calib_date = {}
    ...
    def __getitem__(self, idx):
        ...
        sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib, 'img_path': img_path,
...
class DatasetLidarCameraNuscenes(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, use_reflectance=False,
        ...
        if self.split == 'train':
            self.nusc = NuScenes(version='v1.0-trainval', dataroot=self.root_dir, verbose=True)
        elif self.split == 'val' :
            self.nusc = NuScenes(version='v1.0-mini', dataroot=self.root_dir, verbose=True)
        ...
        if split == 'val' or split == 'test':
            val_RT_file = os.path.join(dataset_dir,
                                       f'val_RT_nuscenes_train_val_seq{val_sequence}_{max_r:.2f}_{max_t:.2f}.csv')
            if os.path.exists(val_RT_file):
                print(f'VAL SET: Using this file: {val_RT_file}')
                df_test_RT = pd.read_csv(val_RT_file, sep=',')
                for index, row in df_test_RT.iterrows():
