...
#                 "load_weights_path" : './COTR/out/default/checkpoint.pth.tar' ,
               "load_weights_path" : '/home/ubuntu/work/autocalib/considering_project/COTR/out/model/76000_0.0008val_checkpoint.pth.tar',
...
#         ############# end of display input signal ###################
        ...
        # depth_input = F.relu(F.max_pool2d(self.stn.conv1(depth_input), 2))
        # depth_input = F.relu(F.max_pool2d(self.stn.conv2_drop(self.stn.conv2(depth_input)), 2))
        # depth_input = depth_input.view(-1, 320)
        # depth_input = F.relu(self.stn.fc1(depth_input))
        # depth_input = F.dropout(depth_input, training=self.training)
        # depth_input = self.stn.fc2(depth_input)
        # depth_input= F.log_softmax(depth_input, dim=1)
