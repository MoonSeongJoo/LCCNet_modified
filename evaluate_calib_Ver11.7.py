...
weights = [
   './checkpoints/kitti/odom/val_seq_07/models/checkpoint_r10.00_t0.25_e37_1.269.tar',
...
def restore_extrinsic_matrix(pos_final_inverse):
    # pos_final_inverse: [batch, 4, 4]
    R_inv = pos_final_inverse[:, :3, :3]  # rotation part
    T_inv = pos_final_inverse[:, :3, 3]   # translation part

    R_restore = R_inv.transpose(-1,-2)  # transpose the rotation part
    T_restore = -torch.bmm(R_restore, T_inv.unsqueeze(-1)).squeeze(-1)  # new translation part

    result = torch.zeros_like(pos_final_inverse)
    result[:, :3, :3] = R_restore
    result[:, :3, 3] = T_restore
    result[:, 3, 3] = 1.0
...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            # reduce_lidar_resolution(pc_lidar, original_line_num=64, target_line_num=32)

            if _config['max_depth'] < 80.:
                pc_lidar = pc_lidar[:, pc_lidar[0, :] < _config['max_depth']].clone()
            
            depth_gt, gt_uv , gt_z , gt_points_index  = lidar_project_depth(pc_lidar, sample['calib'][idx].cuda(), real_shape) # image_shape
            ...
            pc_rotated = rotate_back(pc_lidar, RT_inv)  # Pc` = RT * Pc
...
#                 pc_input_valid = sample['depth_img_pc_valid'][0]
            ...
            pc_rotated_input.append(resample_pcl_rotate)
            pc_ori_input.append(resample_pcl_origin)
        ...
        pc_ori_input = torch.stack(pc_ori_input)
...
#         print ('------rgb_input shape---------', rgb_input.shape)
        ...
        if _config['save_image']:
            out0 = overlay_imgs(rgb_input[0], lidar_projetion_input)
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                if _config['iterative_method'] == 'single_range' or _config['iterative_method'] == 'single':
                    corrs_pred , cycle , mask ,exp_transl_seq, exp_rot_seq, transl_seq, rot_seq, pos_final, current_source = models[0](pc_rotated_input, sbs_img_masked, queries , corr_target ,pose_source, pose_target)
                elif _config['iterative_method'] == 'multi_range':
                    corrs_pred , cycle , mask ,exp_transl_seq, exp_rot_seq, transl_seq, rot_seq, pos_final, current_source = models[iteration](pc_rotated_input, sbs_img_masked, queries , corr_target ,pose_source, pose_target)
                    print ("enter model inference")
                run_time = time.time() - t1
                
                # ##### display corrs images #############
                # corr_target_cpu =  corr_target
                # img_cpu   =  sbs_img_masked.cpu()
                # corrs_cpu = corrs_pred.cpu().detach().numpy()
                # query_cpu = queries.cpu().detach().numpy()
                ...
                pos_final_restored = restore_extrinsic_matrix(pos_final)
                ...
                RT_predicted = pos_final_restored
                RTs.append(torch.mm(RTs[iteration], RT_predicted[0])) # inv(H_gt)*H_pred_1*H_pred_2*.....H_pred_n
                if iteration == 0:
                    rotated_point_cloud = pc_rotated_input[0]
                else:
                    rotated_point_cloud = rotated_point_cloud

                rotated_point_cloud = rotate_forward(rotated_point_cloud, RT_predicted[0]) # H_pred*X_init

                depth_img_pred, uv_pred, z_pred, pc_pred_valid = lidar_project_depth(rotated_point_cloud, sample['calib'][0], real_shape) # image_shape
                depth_img_pred /= _config['max_depth']
                
                lidarOnImage = torch.cat((uv_pred, z_pred), dim=1)
                dense_depth_pred_img = dense_map(lidarOnImage.T , real_shape[1], real_shape[0] , _config['dense_resoltuion']) # argument = (lidarOnImage.T , 1241, 376 , 8)
                dense_depth_pred_img = dense_depth_pred_img.to(dtype=torch.uint8)
                dense_depth_pred_img_color = colormap(dense_depth_pred_img)
                dense_depth_pred_img_color = F.pad(dense_depth_pred_img_color, shape_pad)

                dense_depth_pred_img_color = F.interpolate(dense_depth_pred_img_color.unsqueeze(0), size=[192, 640], mode="bilinear")
                ...
                # plt.subplot(212)
                # plt.imshow(torchvision.utils.make_grid(dense_depth_pred_img_color).permute(1,2,0).cpu().numpy() , cmap='magma')
                # plt.title("dense_depth_input", fontsize=22)
                # plt.axis('off')        
                ...
                if _config['save_image']:
                    out2 = overlay_imgs(rgb_input[0], lidar_resize)
                    out2 = out2[:376, :1241, :]
